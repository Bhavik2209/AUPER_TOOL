...
GOOGLE_API_KEY = "AIzaSyCcZBP9rnajvNqH-Itvircz1wWnCeDCzQM"
...
def create_zip_file(audio_file, timestamps_file):
    zip_buffer = io.BytesIO()
    with zipfile.ZipFile(zip_buffer, "a", zipfile.ZIP_DEFLATED, False) as zip_file:
        for file in [audio_file, timestamps_file]:
        ...
        if uploaded_file is not None:
            ...
            if st.button("Convert to Audio", key="convert"):
                with st.spinner("Converting... This may take a few minutes."):
                    ...
                    with col3:
                        zip_buffer = create_zip_file(output_file, timestamps_file)
                        st.download_button(
                            label="Download All (ZIP)",
                            data=zip_buffer.getvalue(),
                            file_name="audio_and_timestamps.zip",
